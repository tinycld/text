...
### CI / build
- [x] **IMPORTANT** — Go server tests run in CI. `.github/workflows/ci.yml` now has a `go-tests` job that installs Go 1.25, checks out the pinned app shell, links text in (so the `go.mod` `replace tinycld.org/core => ../../tinycld/core/server` resolves), and runs `go test ./...` from `text/server`.
