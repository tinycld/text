...
> On iOS and Android there are no drag handles — tap the image and pick an **S**, **M**, **L**, or **Original** size preset from the sheet that slides up instead. See [Editing images on mobile](help://text:image-on-mobile).
...
- The image toolbar is mouse-driven for now — there are no keyboard shortcuts for the wrap modes or the reset action. On mobile the same wrap modes (**Inline**, **Wrap left**, **Wrap right**, **Break**) are chips in the image sheet — see [Editing images on mobile](help://text:image-on-mobile).
