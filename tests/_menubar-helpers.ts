...
// Aligned with text-document.spec.ts. Realtime sync + .docx parse +
// Y.Doc bootstrap blows past the default 30s under worker contention.
...
export async function waitForEditor(page: Page, timeout = 60_000): Promise<void> {
    await expect(editorRoot(page)).toBeVisible({ timeout })
}

// Opens a freshly-uploaded text document and waits for the editor to
// be ready. Each menubar spec gets its own document so destructive
// items (Rename, Move to trash, Make a copy) can't poison siblings.
export async function openFreshTextDocument(page: Page, label: string): Promise<string> {
    const itemId = await uploadDocxAsDriveItem(`${label}-${Date.now()}.docx`)
    await login(page)
